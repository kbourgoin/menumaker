...
describe('validation utilities', () => {
  ...
  describe('validateDish', () => {
    ...
    it('should throw for missing required fields', () => {
      expect(() => validateDish({ ...validDish, id: '' }))
        .toThrow('Dish ID is required')
      
      expect(() => validateDish({ ...validDish, name: '' }))
        .toThrow('Dish name is required')
      
      expect(() => validateDish({ ...validDish, userId: '' }))
        .toThrow('User ID is required')
    })

    it('should throw for invalid field types', () => {
      expect(() => validateDish({ ...validDish, name: 123 }))
        .toThrow('dish.name.trim is not a function')
      
      expect(() => validateDish({ ...validDish, cuisines: 'Italian' }))
        .toThrow('Cuisines must be an array of strings')
      
      expect(() => validateDish({ ...validDish, timesCooked: -1 }))
        .toThrow('Times cooked cannot be negative')
    })

    it('should validate name length', () => {
      const longName = 'a'.repeat(256)
      expect(() => validateDish({ ...validDish, name: longName }))
        .toThrow('Dish name cannot exceed 255 characters')
    })

    it('should validate cuisines', () => {
      expect(() => validateDish({ ...validDish, cuisines: ['Invalid'] }))
    ...
    it('should validate dates when provided', () => {
      expect(() => validateDish({ ...validDish, createdAt: 'invalid-date' }))
        .toThrow('Invalid created date format')
      
      expect(() => validateDish({ ...validDish, lastMade: 'invalid-date' }))
  ...
  describe('validateMealHistory', () => {
    ...
    it('should throw for missing required fields', () => {
      expect(() => validateMealHistory({ ...validMealHistory, id: '' }))
        .toThrow('Meal history ID is required')
      
      expect(() => validateMealHistory({ ...validMealHistory, dishId: '' }))
        .toThrow('Dish ID is required')
      
      expect(() => validateMealHistory({ ...validMealHistory, userId: '' }))
        .toThrow('User ID is required')
    })

    it('should validate date format', () => {
      expect(() => validateMealHistory({ ...validMealHistory, date: 'invalid-date' }))
    ...
    it('should validate notes length when provided', () => {
      ...
      expect(() => validateMealHistory({ ...validMealHistory, notes: longNotes }))
  ...
  describe('validateSource', () => {
    ...
    it('should throw for missing required fields', () => {
      expect(() => validateSource({ ...validSource, id: '' }))
        .toThrow('Source ID is required')
      
      expect(() => validateSource({ ...validSource, name: '' }))
        .toThrow('Source name is required')
      
      expect(() => validateSource({ ...validSource, type: '' as never }))
        .toThrow('Source type is required')
    })

    it('should validate source type', () => {
      expect(() => validateSource({ ...validSource, type: 'invalid' as never }))
        .toThrow('Source type must be "book" or "website"')
    })

    it('should validate name length', () => {
      const longName = 'a'.repeat(256)
      expect(() => validateSource({ ...validSource, name: longName }))
        .toThrow('Source name cannot exceed 255 characters')
    })

    it('should validate both book and website types', () => {
      const bookSource = { ...validSource, type: 'book' as const }
      const websiteSource = { ...validSource, type: 'website' as const, url: 'https://example.com' }
    ...
    it('should validate URL format for websites', () => {
      const websiteSource = { ...validSource, type: 'website' as const, url: 'invalid-url' }
      expect(() => validateSource(websiteSource))
  ...
  describe('validateProfile', () => {
    ...
    it('should throw for missing required fields', () => {
      expect(() => validateProfile({ ...validProfile, id: '' }))
    ...
    it('should validate username format when provided', () => {
      expect(() => validateProfile({ ...validProfile, username: 'ab' }))
        .toThrow('Username must be at least 3 characters long')
      
      const longUsername = 'a'.repeat(51)
      expect(() => validateProfile({ ...validProfile, username: longUsername }))
        .toThrow('Username cannot exceed 50 characters')
    })

    it('should validate cuisines when provided', () => {
      expect(() => validateProfile({ ...validProfile, cuisines: ['Invalid'] }))
        .toThrow('Invalid cuisine type: Invalid')
    })
  })
  ...
  describe('edge cases and error handling', () => {
    it('should handle malformed data gracefully', () => {
      ...
      expect(() => validateDish(malformedDish as never))
        .toThrow() // Should throw some validation error
    })

    it('should provide specific error messages', () => {
      try {
        validateDish({ id: '', name: 'Test', userId: 'test-user-1', cuisines: [], timesCooked: 0, tags: [], createdAt: '2024-01-01T00:00:00Z' })
      } catch (error) {
