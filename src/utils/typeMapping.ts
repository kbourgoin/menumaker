...
export const mapDishFromSummary = (summary: DBDishSummary): Dish => {
  try {
    if (!summary.id || !summary.name || !summary.user_id) {
      throw new ValidationError(
        "Invalid dish summary: missing required fields"
      );
    }
...
export const mapMealHistoryFromDB = (dbHistory: DBMealHistory): MealHistory => {
  try {
    if (!dbHistory.id || !dbHistory.dishid || !dbHistory.user_id) {
      throw new ValidationError(
        "Invalid meal history record: missing required fields"
      );
    }
 ...
 */
export const mapMealHistoryToDB = (
 ...
 */
export const mapSourceFromDB = (
  dbSource: DBSource | DBSourceExtended
): Source => {
  try {
    if (!dbSource.id || !dbSource.name || !dbSource.user_id) {
      throw new ValidationError(
        "Invalid source record: missing required fields"
      );
    }
 ...
 */
export const mapSourceToDB = (
  source: Partial<Source>
): DBSourceInsert & { url?: string } => {
  try {
...
export const mapTagToDB = (tag: Partial<Tag>): DBTagInsert => {
  try {
    if (!tag.id && (!tag.name || !tag.userId)) {
      throw new ValidationError(
        "Name and userId are required when creating a tag"
      );
    }
...
export const mapProfileToDB = (profile: Partial<Profile>): DBProfileInsert => {
  try {
    if (!profile.id) {
      throw new ValidationError(
        "Id is required when creating/updating a profile"
      );
    }
...
export const mapArrayFromDB = {
  dishes: (
    dbDishes: DBDish[],
    mealHistoryMap?: Map<string, DBMealHistory[]>
  ): Dish[] => {
