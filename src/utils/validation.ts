...
export const isValidTagCategory = (
  category: string
): category is "cuisine" | "general" => {
...
export const validateDish = (dish: Partial<Dish>): void => {
  ...
  if (
    dish.location &&
    dish.location.length > DB_CONSTRAINTS.DISH.LOCATION_MAX_LENGTH
  ) {
    throw new ValidationError(
...
export const validateMealHistory = (
  mealHistory: Partial<MealHistory>
): void => {
  // Required fields
  if (!mealHistory.id) {
    throw new ValidationError(
      "Meal history ID is required",
      "id",
      "REQUIRED_FIELD"
    );
  }
  ...
  if (!isValidUUID(mealHistory.id)) {
    throw new ValidationError(
      "Invalid meal history ID format",
      "id",
      "INVALID_FORMAT"
    );
  }
  ...
  if (
    mealHistory.notes &&
    mealHistory.notes.length > DB_CONSTRAINTS.MEAL_HISTORY.NOTES_MAX_LENGTH
  ) {
    throw new ValidationError(
...
export const validateSource = (source: Partial<Source>): void => {
  ...
  if (!isValidSourceType(source.type)) {
    throw new ValidationError(
      'Source type must be "book" or "website"',
      "type",
      "INVALID_VALUE"
    );
  }
  ...
  if (
    source.description &&
    source.description.length > DB_CONSTRAINTS.SOURCE.DESCRIPTION_MAX_LENGTH
  ) {
    throw new ValidationError(
  ...
  if (source.type === "website" && source.url) {
    try {
      ...
      throw new ValidationError(
        "Invalid URL format for website source",
        "url",
        "INVALID_FORMAT"
      );
    }
...
export const validateTag = (tag: Partial<Tag>): void => {
  ...
  if (tag.category && !isValidTagCategory(tag.category)) {
    throw new ValidationError(
      'Tag category must be "cuisine" or "general"',
      "category",
      "INVALID_VALUE"
    );
  }
  ...
  if (
    tag.description &&
    tag.description.length > DB_CONSTRAINTS.TAG.DESCRIPTION_MAX_LENGTH
  ) {
    throw new ValidationError(
