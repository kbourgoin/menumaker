...
const StatsCard = React.memo<StatsCardProps>(({ stats, isLoading }) => {
  ...
  const cuisinesToShow = showAllCuisines ? cuisineData : cuisineData.slice(0, 5);
          ...
          <>
            <div className="grid grid-cols-2 gap-3">
              <div className="flex justify-between items-center">
                <span className="text-muted-foreground text-sm">Total Dishes:</span>
                <span className="font-medium">{stats.totalDishes}</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-muted-foreground text-sm">Times Cooked:</span>
            ...
            {topDishes.length > 0 && (
              <div>
                <span className="text-sm text-muted-foreground mb-2 block">Top 5 Most Cooked:</span>
                <div className="space-y-1">
                  {topDishes.map((dish, index) => (
                    <div key={dish.id} className="flex justify-between items-center text-sm">
                      ...
                      <span className="text-muted-foreground ml-2">{dish.timesCooked}x</span>
                    </div>
                  ))}
                </div>
              </div>
            ...
            {cuisineData.length > 0 && (
              <div>
                <span className="text-sm text-muted-foreground mb-2 block">Top 5 Cuisines:</span>
                <div className="space-y-1.5">
                  {cuisinesToShow.map((item, index) => {
                    const total = cuisineData.reduce((sum, curr) => sum + (curr.value as number), 0);
                    const percentage = Math.round(((item.value as number) / total) * 100);
                    return (
