  ...
  const mergeSources = useMutation({
    mutationFn: async ({
      ...
      if (!sourceToMergeId || !targetSourceId) {
        throw createSourceMergeError(
          "Both source IDs are required for merge operation"
        );
      }
      ...
      try {
        ...
        const { data: sourceToMerge, error: sourceToMergeError } =
          await supabase
            .from("sources")
            .select("id, name")
        ...
        if (sourceToMergeError) {
          throw createSourceMergeError(
            `Failed to verify source to merge: ${sourceToMergeError.message}`,
            sourceToMergeError
          );
        }
        ...
        if (!sourceToMerge) {
          throw createSourceMergeError(
            "Source to merge not found or you do not have permission to access it"
          );
        }
        ...
        if (targetSourceError) {
          throw createSourceMergeError(
            `Failed to verify target source: ${targetSourceError.message}`,
            targetSourceError
          );
        }
        ...
        if (!targetSource) {
          throw createSourceMergeError(
            "Target source not found or you do not have permission to access it"
          );
        }
        ...
        if (dishesError) {
          throw createSourceMergeError(
            `Failed to fetch dishes to merge: ${dishesError.message}`,
            dishesError
          );
        }
        ...
        if (dishesData && dishesData.length > 0) {
          ...
          if (updateError) {
            throw createSourceMergeError(
              `Failed to update dishes during merge: ${updateError.message}`,
              updateError
            );
          }
        }
        ...
        if (deleteError) {
          throw createSourceMergeError(
            `Failed to delete merged source: ${deleteError.message}`,
            deleteError
          );
        }
        ...
        throw error instanceof Error
          ...
          : createSourceMergeError(
              "Unknown error occurred while merging sources"
            );
      }
    },
    onSuccess: data => {
      logger.operation(
        `Successfully merged "${data.sourceToMergeName}" into "${data.targetSourceName}", affecting ${data.affectedDishesCount} dishes`,
        "source-merge"
      );
