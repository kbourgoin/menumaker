...
const createSourceError = (message: string, originalError?: unknown): SourceQueryError => {
...
const handleAuthError = (error: unknown): never => {
  if (isErrorWithCode(error) && 
    ...
    throw createSourceError('Authentication failed. Please log in again.', error);
  }
  throw error;
};
...
export function useSourceQueries() {
  ...
  const getSource = async (id: string): Promise<Source | null> => {
    ...
    try {
      ...
      if (error) {
        throw createSourceError(`Failed to fetch source: ${error.message}`, error);
      }
      ...
      throw error instanceof Error ? error : createSourceError('Unknown error occurred while fetching source');
    }
  };
  ...
  const getDishesBySource = async (sourceId: string): Promise<Dish[]> => {
    ...
    try {
      ...
      if (error) {
        throw createSourceError(`Failed to fetch dishes by source: ${error.message}`, error);
      }
      ...
      if (dishIds.length > 0) {
        try {
          ...
          if (historyError) {
            ...
            console.warn('Failed to fetch meal history for dishes:', historyError);
          } else if (historyData) {
      ...
      return dishesData ? dishesData.map(dish => mapDishFromDB(dish, mealHistoryByDish[dish.id] || [])) : [];
    } catch (error) {
      console.error('Error in getDishesBySource:', error);
      // Re-throw for proper error handling
      throw error instanceof Error ? error : createSourceError('Unknown error occurred while fetching dishes by source');
    }
  };
  ...
  const findSourceByName = async (name: string, excludeId?: string): Promise<Source | null> => {
    ...
    try {
      ...
      if (error) {
        throw createSourceError(`Failed to find source by name: ${error.message}`, error);
      }
      ...
      // Re-throw for proper error handling
      throw error instanceof Error ? error : createSourceError('Unknown error occurred while finding source by name');
    }
  };
