...
export function useMealHistoryQueries() {
          ...
          const data = await retryOperation(
            async () => {
              ...
              if (error) {
                ...
                logError(appError, 'useMealHistoryQueries:mealHistoryForDish:fetch');
                throw error;
              }
        ...
        try {
          ...
          const data = await retryOperation(
            async () => {
              ...
              if (error) {
                const appError = classifyError(error);
                logError(appError, 'useMealHistoryQueries:allMealHistory:fetch');
                throw error;
              }
  ...
  const getMealHistoryForDish = async (dishId: string): Promise<MealHistory[]> => {
    try {
      if (!dishId?.trim()) {
        const validationError = classifyError(new Error('Dish ID is required'));
        logError(validationError, 'useMealHistoryQueries:getMealHistoryForDish:validation');
        return [];
      }
      ...
      const data = await retryOperation(
        async () => {
          ...
          if (error) {
            const appError = classifyError(error);
            logError(appError, 'useMealHistoryQueries:getMealHistoryForDish:fetch');
            throw error;
          }
      ...
      const appError = classifyError(error);
      logError(appError, 'useMealHistoryQueries:getMealHistoryForDish:top-level');
      return [];
    }
  };
