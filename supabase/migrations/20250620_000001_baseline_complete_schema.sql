...
-- Note: This baseline migration includes all tables, functions, policies, indexes, and views
-- as they existed in production at the time of extraction
...
CREATE OR REPLACE FUNCTION "public"."refresh_dish_summary_secure"() RETURNS "void"
    ...
    AS $$
BEGIN
    -- Refresh the materialized view (use CONCURRENTLY only if already populated)
    BEGIN
